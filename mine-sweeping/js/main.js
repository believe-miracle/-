function Mine(tr,td,mineNum){
	...
	this.allRight = false; // 右击标的小红旗是否都是雷 用来判断用户是否游戏成功
	
	this.parent = document.querySelector('.gameBox');
}
//生成n个不重复的数字
Mine.prototype.randomNum = function(){
	var square = new Array(this.td*this.td);//生成一个空数组 但是有长度 长度为格子的总数
...
Mine.prototype.init = function(){
	...
	for (var i=0;i<this.tr;i++) {
		...
		for (var j=0;j<this.td;j++) {
			...
			//取一个方块在数组里的数据要使用行与列的形式去取 找方块周围的方块的时候要用坐标的形式去取 行与列的形式和坐标的x和y刚好是相反的
			if(rn.indexOf(++n) != -1){
				//如果这个条件成立 说明现在循环到的这个索引在雷的数组里找到了 那就代表这个索引对应的是个雷
...
Mine.prototype.createDom = function(){
	...
	for (var i=0;i<this.tr;i++) {//行
		...
		for (var j=0;j<this.td;j++) {//列
			...
			domTd.pos = [i,j]; //把格子对应的行与列存到格子上 为了下面通过这个值去数组里去到对应的数值
			domTd.onmousedown = function(){
				This.play(event,this) ; // This指的是实例对象
			}
			
			this.tds[i][j]=domTd; //在这里是把所有创建的td添加到数组当中
	...
	this.parent.innerHTML = ''; //避免多次点击创建多次
...
Mine.prototype.play = function(ev,obj){
	var This = this;
	if(ev.which == 1 && obj.className != 'flag'){ //后面的条件是限制用户标完小红旗后就不能左键点击
		//点击的是左键
		//console.log(obj);
				...
				/*
					用户点到了数字0
						...
						2.找四周
							...
							2.如果值为0
								1.显示自己
								2.显示四周(如果四周的值不为0，那就显示到这里，不需要再找了)
				*/
			   obj.innerHTML = '';  //如果数字为0的话就不显示
					...
					for (var i=0;i<around.length;i++) {
						...
						if(This.squares[x][y].value == 0){
							...
							if(!This.tds[x][y].check){
								//给对应的td添加一个属性，这条属性用于决定这个格子有没有被找过 如果找过 他的值就为true 下一次就不会再找了
	...
	if(ev.which == 3){
		...
		if(this.surplusMine == 0){
			//剩余的雷的数量为0 表明用户已经标完小红旗了 这时候要判断游戏是成功还是结束
			if(this.allRight){
				alert('恭喜你，游戏通过！');
			}else{
				alert('游戏失败');
